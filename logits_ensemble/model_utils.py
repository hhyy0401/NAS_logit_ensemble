...
def gnn_train(model, loader, optimizer, device, loss_type="exp", idx=-1, multi=True):
    model.train()
    total_loss = 0
    ...
    for data in loader:
        ...
        output = model(data.x, data.edge_index, data.batch, False)

        if idx>-1:
            y = data['logits_'+str(idx)].float().reshape(output.shape[0], -1)
        else:
...
@torch.no_grad()
def gnn_eval(model, loader, device, loss_type="exp", idx=-1, multi=True):
    model.eval()
    total_loss = 0
    kl_loss = torch.nn.KLDivLoss(reduction="batchmean")
    for data in loader:
...
'''
def mlp_train(model, loader, optimizer, device, multi=True, loss_type="exp"):
    model.train()
    total_loss = 0
    for data in loader:
        optimizer.zero_grad()
        output = model(data[0], embedding=False)
    
        loss = F.mse_loss(output, data[1])
        
        loss.backward()
        optimizer.step()
        total_loss += float(loss) * data[0].shape[0]
        #print(data[1], output)

    return total_loss / len(loader)

@torch.no_grad()
def mlp_eval(model, loader, device, multi=True, loss_type="mse"):
    model.eval()
    total_loss = 0
